    ...
    >
      <div className="mx-auto max-w-content px-gutter">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-center text-primary/70">
          Occudule is built with the same obsessive clarity you see in leading finance dashboards—
          applied to the inbox you actually live in.
        </p>
        <div className="mt-12 grid gap-5 sm:grid-cols-3">
          {metrics.map((m) => (
            <div
              key={m.label}
              className="rounded-card border border-border bg-surface-muted/60 p-6 text-center shadow-card"
            >
              <p className="text-sm text-primary/65">{m.label}</p>
              <p className="mt-2 text-3xl font-bold tracking-tight text-primary md:text-4xl">
                {m.value}
              </p>
              <p className="mt-1 text-sm font-semibold text-success">{m.delta}</p>
              <p className="mt-2 text-xs text-primary/50">{m.note}</p>
            </div>
          ))}
        </div>
        <div className="mt-14 rounded-card border border-dashed border-border bg-surface-muted/40 px-6 py-8">
          <p className="text-center text-xs font-semibold uppercase tracking-widest text-primary/45">
            Proud partners of teams & schools piloting Occudule
          </p>
          <ul className="mt-6 flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
            {logos.map((name) => (
              <li key={name}>
                <span className="text-sm font-semibold tracking-tight text-primary/40">{name}</span>
              </li>
            ))}
          </ul>
        </div>
