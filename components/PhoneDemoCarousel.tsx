...
type PhoneDemoCarouselProps = {
  ...
  triggerHint?: string;
  /** Phone preview only — for sidebar placement beside section text */
  previewOnly?: boolean;
};

export function PhoneDemoCarousel({
  images,
  dialogTitle,
  dialogLabel,
  triggerAriaLabel,
  ...
  const showInlineCopy = isFull && title && body && !previewOnly;
  const showTriggerCopy = !previewOnly && (!isFull || !showInlineCopy);
  const showBelowPreview =
    previewOnly && (triggerAccent || triggerHeading || triggerHint);
  ...
  return (
    <>
      <button
        ...
        className={
          previewOnly
            ? "group relative aspect-[9/16] w-[200px] overflow-hidden transition hover:opacity-90 sm:w-[240px]"
            : isFull
        ...
        aria-label={isFull || previewOnly ? triggerAriaLabel : undefined}
      >
        {showInlineCopy ? (
        ...
        {showBelowPreview ? (
          <span className="w-full">
            ...
            {triggerHeading ? (
              <span className="mt-1 block text-xs font-semibold text-white group-hover:text-accent sm:text-sm">
                {triggerHeading}
              </span>
            ) : null}
            {triggerHint ? (
              <span className="mt-0.5 block text-[11px] text-white/50 sm:text-xs">{triggerHint}</span>
            ) : null}
          </span>
        ) : null}
        {showTriggerCopy && (triggerAccent || triggerHeading || triggerHint) ? (
          <span className="min-w-0 flex-1">
            {triggerAccent ? (
              <span className="block text-xs font-medium tracking-wide text-accent sm:text-sm">
                {triggerAccent}
              </span>
            ) : null}
            {triggerHeading ? (
              <span className="mt-1 block text-sm font-semibold text-white group-hover:text-accent sm:text-base">
                {triggerHeading}
              </span>
            ) : null}
            {triggerHint ? (
              <span className="mt-1 block text-xs text-white/50 sm:text-sm">{triggerHint}</span>
            ) : null}
          </span>
        ) : null}
        {!isFull ? (
              ...
              <div className="relative z-10 w-full max-w-[min(360px,calc(100vw-2rem))]">
                ...
                >
                  <div className="border-b border-white/10 px-5 py-4">
                    {dialogLabel ? (
                      <p className="text-xs font-medium tracking-wide text-accent">{dialogLabel}</p>
                    ) : null}
                    <h3
                      id={titleId}
                      className={`text-lg font-semibold text-white ${dialogLabel ? "mt-1" : ""}`}
                    >
                      {dialogTitle}
                    </h3>
